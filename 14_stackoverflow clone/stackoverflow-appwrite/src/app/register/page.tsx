...
import { MessageSquareCode, Lock, Mail, User, AlertCircle, Loader2 } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const regRes = await createAccount(name, email, password);
    if (regRes.success) {
      ...
      setLoading(false);
      setErrorMsg(regRes.error?.message || "Failed to create account. Please try again.");
    }
  };
      ...
      <div className="w-full max-w-md space-y-8 bg-white p-8 rounded-xl border border-zinc-200 shadow-sm dark:bg-zinc-900 dark:border-zinc-800">
        <div className="text-center">
          ...
          <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
            Join the community to ask questions, upvote, and post answers.
          </p>
        </div>
